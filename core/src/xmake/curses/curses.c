...
static int xm_curses_window_attroff(lua_State *lua) {
    WINDOW *w = xm_curses_window_check(lua, 1);
    int attrs = luaL_checkinteger(lua, 2);
...
static int xm_curses_window_copywin(lua_State *lua) {
    ...
    int sminrow = luaL_checkinteger(lua, 3);
    int smincol = luaL_checkinteger(lua, 4);
    int dminrow = luaL_checkinteger(lua, 5);
    int dmincol = luaL_checkinteger(lua, 6);
    int dmaxrow = luaL_checkinteger(lua, 7);
    int dmaxcol = luaL_checkinteger(lua, 8);
...
static int xm_curses_mousemask(lua_State *lua) {
    mmask_t m = luaL_checkinteger(lua, 1);
    mmask_t om;
    m = mousemask(m, &om);
...
static int xm_curses_init_pair(lua_State *lua) {
    short pair = luaL_checkinteger(lua, 1);
    short f    = luaL_checkinteger(lua, 2);
    short b    = luaL_checkinteger(lua, 3);
...
static int xm_curses_curs_set(lua_State *lua) {
    int vis   = luaL_checkinteger(lua, 1);
...
static int xm_curses_newpad(lua_State *lua) {
    int nlines = luaL_checkinteger(lua, 1);
    int ncols  = luaL_checkinteger(lua, 2);
